...
class TestBattery:
    ...
    def test_charge_should_set_power_on_charge_hour(
        self, battery_one_kw_one_kwh: Battery
    ):
        """Test power is set when charging the battery"""
        charge_hour = ChargeHour(hour=0, import_price=2, export_price=1, power=0)
        assert charge_hour.get_power_watts() == 0
        battery_one_kw_one_kwh.charge_max_power_for_one_hour(charge_hour)
        assert charge_hour.get_power_watts() == -1000

    def test_discharge_should_set_power_on_charge_hour(
        self, battery_one_kw_one_kwh: Battery
    ):
        """Test power is set when charging the battery"""
        charge_hour = ChargeHour(hour=0, import_price=2, export_price=1, power=0)
        ...
        battery_one_kw_one_kwh.discharge_max_power_for_one_hour(charge_hour)
        assert charge_hour.get_power_watts() == 1000
